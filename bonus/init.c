...
/*   Updated: 2023/12/23 16:10:58 by ayal-ras         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_set(t_fdf *current, t_fdf *next, t_fdf *data)
{
	...
	current->matrix.x -= (data->scale * 15) / 2;
	current->matrix.y -= (data->scale * 15) / 2;
	next->matrix.x -= (data->scale * 15) / 2;
	next->matrix.y -= (data->scale * 15) / 2;
	rotate_x(&current->matrix.y, &current->matrix.z, data->angle_x);
	rotate_x(&next->matrix.y, &next->matrix.z, data->angle_x);
	rotate_y(&current->matrix.x, &current->matrix.z, data->angle_y);
	rotate_y(&next->matrix.x, &next->matrix.z, data->angle_y);
	rotate_z(&current->matrix.x, &current->matrix.y, data->angle_z);
	rotate_z(&next->matrix.x, &next->matrix.y, data->angle_z);
